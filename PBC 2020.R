...
nms = read.delim("ebd_IN_202001_202012_relDec-2020.txt", nrows = 1, sep = "\t", header = T, quote = "", stringsAsFactors = F, 
                 na.strings = c(""," ",NA))
nms = names(nms)
...
data1 = read.delim("ebd_IN_202001_202012_relDec-2020.txt", colClasses = nms, sep = "\t", header = T, quote = "", 
...
data = data %>% filter(OBSERVATION.DATE %in% c("2020-01-16","2020-01-17","2020-01-18","2020-01-19"))
...
write.csv(temp,"PBCspecieslist2020.csv", row.names=FALSE)
...
datax = data %>%
  ...
  arrange(DISTRICT)
write.csv(datax,"PBCbirdersperdistrict2020.csv", row.names=FALSE)
...
datax = data %>%
  ...
  arrange(desc(lists))
write.csv(datax,"PBClistsperdistrict2020.csv", row.names=FALSE)
...
datax = data %>%
  ...
  arrange(desc(lists))
write.csv(datax,"PBClistsperday2020.csv", row.names=FALSE)
...
write.csv(datax,"PBClistsperdaydistrict2020.csv", row.names=FALSE)
...
datax = data %>%
  ...
  arrange(desc(freq))
write.csv(datax,"PBCcommonspecies2020.csv", row.names=FALSE)
...
mapshot(a, "PBC_districts_2020.html")
...
east = c("Thiruvallur","Chennai","Kancheepuram","Viluppuram","Cuddalore","Nagapattinam",
...
write.csv(regionsummary,"PBC_REGIONsummary2020.csv",row.names = F)
...
write.csv(cosp,"PBC_CommonSpeciesbyRegion2020.csv",row.names = F)
...
n1 = "PBCpointdistrictmap2020.jpg"
