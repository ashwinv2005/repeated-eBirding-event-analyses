...
save(data, file = "allGBBCdata.RData")
...
######################################### get campus summaries over years

cbc = read.csv("CBC_2020.csv")
locs = cbc$D..Campus.Hotspot.ID

cbcdata = alldata %>% filter(LOC_ID %in% locs)
cbcdata1 = alldata1 %>% filter(LOC_ID %in% locs)

clists = cbcdata %>%
  group_by(year,ST_NM,NAME) %>% summarize(participants = n_distinct(P_NAME), lists = n_distinct(SUB_ID), 
                               uniquelists = n_distinct(group.id))

cspecs = cbcdata1 %>%
  group_by(year,ST_NM,NAME) %>% summarize(species = n_distinct(PRIMARY_COM_NAME))

campussummary = left_join(clists,cspecs)

campussummary = campussummary %>% arrange(desc(participants))

WII = campussummary %>% filter(NAME == "Wildlife Institute of India Campus") %>% arrange(desc(year))
NCR = campussummary %>% filter(ST_NM %in% c("NCT of Delhi","Haryana"), year == 2020)

clists = cbcdata %>%
  group_by(year,NAME,P_NAME) %>% summarize(lists = n_distinct(SUB_ID), 
                                          uniquelists = n_distinct(group.id))

cspecs = cbcdata1 %>%
  group_by(year,NAME,P_NAME) %>% summarize(species = n_distinct(PRIMARY_COM_NAME))

campussummary = left_join(clists,cspecs)

campussummary = campussummary %>% arrange(desc(lists))

WIIpeople = campussummary %>% filter(NAME == "Wildlife Institute of India Campus") %>% 
  arrange(desc(P_NAME,year,lists))
...
write.csv(NCR,"NCR.csv",row.names = F)
write.csv(WII,"WII.csv",row.names = F)
write.csv(WIIpeople,"WIIpeople.csv",row.names = F)
...
sums1 = alldata %>%
  group_by(year) %>% summarize(num = n_distinct(SUB_ID)) %>% ungroup %>%
...
ggp1 = ggp +
  ...
  scale_colour_manual(breaks = c("Checklists","Person Hours"), 
                      values = cols[c(1:2)]) +
  scale_x_continuous(breaks = c(2013,2014,2015,2016,2017,2018,2019,2020)) +
  scale_y_continuous(breaks = c(0,3000,6000,9000,12000,15000,18000,21000,24000,27000), 
                     labels = c("0","3,000","6,000","9,000","12,000","15,000","18,000","21,000","24,000","27,000"),
                     limits = c(0,28000))
...
dedpart = data %>% 
  filter(DURATION_HRS >= 0.25) %>%
  group_by(P_NAME) %>% summarize(lists = n_distinct(SUB_ID)) %>%
  filter(lists >= 100)

datajoin = data %>%
  group_by(P_NAME) %>% slice(1) %>% ungroup

dedpart = left_join(dedpart,datajoin)

dedpart = dedpart %>%
  select(P_NAME,lists,ST_NM)

write.csv(dedpart,"topparticipants.csv",row.names = F)
