...
nms = read.delim("ebd_IN_202102_202102_relFeb-2022.txt", nrows = 1, sep = "\t", header = T, quote = "", stringsAsFactors = F, 
                 na.strings = c(""," ",NA))
nms = names(nms)
...
data1 = read.delim("ebd_IN_202102_202102_relFeb-2022.txt", colClasses = nms, sep = "\t", header = T, quote = "", 
                   stringsAsFactors = F, na.strings = c(""," ","null",NA))

## choosing important columns required for further analyses

data = data1
data = data %>% filter(OBSERVATION.DATE %in% c("2021-02-12","2021-02-13","2021-02-14","2021-02-15"))
...
write.csv(temp,"GBBCspecieslist2021.csv", row.names=FALSE)
...
datax = datax %>%
  group_by(FIRST.NAME,LAST.NAME) %>% summarize(lists15mins = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>% ungroup %>%
  arrange(desc(lists15mins)) %>% slice(1:38)
datax = datax[-c(1,2,3,4,25,27,32,34),]

write.csv(datax,"GBBCtop30birders2021.csv", row.names=FALSE)

########## Top birder in every state

datax = data %>% filter(ALL.SPECIES.REPORTED == 1, DURATION.MINUTES >= 14.5) %>%
  filter(!obs.id.num %in% c(949737,879790,1227617,2238566,852457,654194,2213036,2228816))
datax = datax %>%
...
write.csv(datax,"GBBCtopbirderperstate2021.csv", row.names=FALSE)
...
write.csv(datax,"GBBCbirdersperstate2021.csv", row.names=FALSE)
...
write.csv(datax,"GBBClistsperdistrict2021.csv", row.names=FALSE)

datax = data %>%
  group_by(ST_NM) %>% summarize(lists = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  arrange(desc(lists)) %>% filter(!is.na(ST_NM))
write.csv(datax,"GBBClistsperstate2021.csv", row.names=FALSE)
...
datax = data %>%
  ...
  arrange(desc(lists))
write.csv(datax,"GBBClistsperday2021.csv", row.names=FALSE)
...
write.csv(datax,"GBBClistsperdaystate2021.csv", row.names=FALSE)
...
datax = data %>%
  ...
  arrange(desc(freq))
write.csv(datax,"GBBCcommonspecies2021.csv", row.names=FALSE)
...
mapshot(a, "GBBC_districts_2021.html")
...
temp$GBBCweb = '<a href = "https://ebird.org/gbbc/region/IN/regions?yr=EBIRD_GBBC_2021&m="> Click on the State for more information </a>'
...
mapshot(b, "GBBC_states_2021.html")

c = a + b

mapshot(c, "GBBC_states_districts_2021.html")
...
write.csv(regionsummary,"GBBC_REGIONsummary_2021.csv",row.names = F)
...
write.csv(cosp,"GBBC_CommonSpeciesbyRegion_2021.csv",row.names = F)
...
extra = data5[1:2,]
extra$ST_NM = c("MIZORAM","LAKSHADWEEP")
extra$region = "East"
...
ggp1 = ggp +
  ...
  scale_y_continuous(breaks = c(0,6000,12000,18000,24000,30000), 
                     labels = c("0","6,000","12,000","18,000","24,000","30,000"),
                     limits = c(0,33000))
...
ggp2 = ggp +
  ...
  scale_colour_manual(breaks = c("Participants"), 
                      values = cols[c(1)]) +
  scale_x_continuous(breaks = c(2013,2014,2015,2016,2017,2018,2019,2020,2021)) +
  scale_y_continuous(breaks = seq(0,3000,500), labels = c("0","500","1000","1500","2000","2,500","3,000"))
...
ggp4 = ggp +
  ...
  scale_colour_manual(breaks = c("Districts"), 
                      values = cols[c(1)]) +
  scale_x_continuous(breaks = c(2013,2014,2015,2016,2017,2018,2019,2020,2021)) +
  scale_y_continuous(breaks = seq(100,450,50), labels = c("100","150","200","250","300","350","400","450"),
                     limits = c(100,450))
...
freqs = dataall %>%
  ...
  group_by(DISTRICT) %>% mutate(distlist = n_distinct(group.id)) %>% ungroup %>%
  filter(distlist > 10) %>%
  group_by(year,COMMON.NAME,DISTRICT) %>% summarize(freq = n_distinct(group.id)/max(distlist)) %>% ungroup %>%
  group_by(year) %>% mutate(ndist = n_distinct(DISTRICT)) %>% ungroup %>%
  group_by(COMMON.NAME,year) %>% summarize(freq = sum(freq)/max(ndist)) %>% ungroup 
...
print(ggp1)
ggsave(file="effort.jpg", units="in", width=10, height=7)

print(ggp2)
ggsave(file="participation.jpg", units="in", width=10, height=7)

print(ggp3)
ggsave(file="species.jpg", units="in", width=10, height=7)

print(ggp4)
ggsave(file="spread.jpg", units="in", width=10, height=7)
...
campus = read.csv("2021campushotspots.csv")
names(campus)[1] = "LOCALITY.ID"

data = left_join(data,campus)

data1 = data %>% distinct(LOCALITY,LOCALITY.ID,dup) %>% group_by(dup) %>% summarize(n = n_distinct(LOCALITY.ID))
data1[data1$n > 1,]

reps = na.omit(data1[data1$n > 1,]$dup)

data2 = data %>% distinct(LOCALITY,LOCALITY.ID,dup) %>% filter(dup %in% reps) %>% arrange(dup)

list2 = data.frame(dup = c(2,34,118,122),LOCALITY = c("Guahati University Campus",
                                                      "Bhusawal Thermal Power Station (BTPS)",
                                                      "Indian Institute of Science (IISc) Campus",
                                                      "Kerala Agricultural University"))

data3 = data %>% filter(!dup %in% reps) %>% filter(!is.na(dup))
list1 = data3 %>% distinct(dup,LOCALITY)

list = rbind(list1,list2)

names(list)[2] = "campus"

data4 = left_join(data,list)

datafinal = data4 %>% filter(!is.na(dup))
...
write.csv(temp,"CBCresults.csv", row.names=FALSE)

names(temp)[1] = "Campus"
latlong = datafinal %>% distinct(campus,LONGITUDE,LATITUDE)
names(latlong)[1] = "Campus"
