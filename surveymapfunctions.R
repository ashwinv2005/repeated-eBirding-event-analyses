...
## 'rawpath' is the filename of the data csv; owndata = T implies that the file is an own data download; 
## (cont'd) ensure that this file is in the working directory
## 'path' and 'name' refer to the folder and file names of the shapefile data; ensure that the data is 
## (cont'd) in the working directory
...
surveymaps = function(species, filter = c("Periyar Tiger Reserve"), rawpath = "MyEBirdData.csv", 
                      owndata = T, path = "Division@Range6", name = "Division&Range6", 
                      gridsize = 3, smooth = F, h = 0.1, cutoff = 2, showempty = T)
{
  data = readcleanrawdata(rawpath, owndata)
  data = data %>%
  ...
  tempmap = readOGR("Division@Range6","Division&Range6")
