...
nms = read.delim("ebd_IN_202202_202202_relFeb-2022.txt", nrows = 1, sep = "\t", header = T, quote = "", stringsAsFactors = F, 
                 na.strings = c(""," ",NA))
nms = names(nms)
...
data1 = read.delim("ebd_IN_202202_202202_relFeb-2022.txt", colClasses = nms, sep = "\t", header = T, quote = "", 
                   stringsAsFactors = F, na.strings = c(""," ","null",NA))

## choosing important columns required for further analyses

data = data1
data = data %>% filter(OBSERVATION.DATE %in% c("2022-02-18","2022-02-19","2022-02-20","2022-02-21"))
